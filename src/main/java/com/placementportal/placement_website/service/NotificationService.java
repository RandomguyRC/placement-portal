...
private List<Student> getEligibleStudents(JobListing job) {
    ...
    try {
        ...
        // Use "cpi" instead of "min_cpi"
        double requiredCpi = root.has("cpi") ? root.get("cpi").asDouble(0.0) : 0.0;

        // Use "branch" instead of "branches"
        Set<String> allowedBranches = new HashSet<>();
        if (root.has("branch") && root.get("branch").isArray()) {
            for (JsonNode b : root.get("branch")) {
                allowedBranches.add(b.asText().trim().toLowerCase());
            }
        }
